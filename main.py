...
# object for database
data = Database(db='expense_records.db')
...
def save_record():
    global data
    data.insert_record(item_name=item_name.get(), item_price=item_amt.get(), purchase_date= transaction_date.get())
...
def fetch_records():
    f = data.fetchRecord('select rowid, * from expense_records')
    global count
    for rec in f:
...
def totalBalance():
    f = data.fetchRecord(query="Select sum(item_price) from expense_records")
    for i in f:
        for j in i:
